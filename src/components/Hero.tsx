...
export default function Hero() {
  ...
  return (
    <section className="relative top-0 left-0 w-full h-[90vh] flex flex-col justify-center items-center text-center text-black px-4 sm:px-6 md:px-8">
      <div className="flex flex-col justify-center items-center h-full pt-8 gap-6 max-w-full">
        {/* Eyebrow Section */}
        <p className="text-sm md:text-base tracking-wide text-black/60 mb-2">
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 mt-2 w-full max-w-md">
          ...
          <div
            className="w-full sm:w-auto bg-white/80 rounded-xl text-black p-4 flex justify-center hover:bg-black hover:text-white transition"
            onClick={() => scrollToPlans(1000)}
          >
            <Button className="w-full sm:w-auto text-center">See Pricing</Button>
          </div>

          {/* Book a 15-min Intro Call Section */}
          <div className="flex items-center gap-4 border-2 border-black/20 border-dashed rounded-lg px-3 py-2 w-full sm:w-auto justify-between">
            {/* Left: Avatar / emoji */}
